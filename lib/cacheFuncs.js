...
const printMappings = (memLocation, wordsInBlock, wordsInMemory, wordsInCache) => {
    ...
    layoutArray.forEach(layoutMode => {
        ...
        console.log('Memory in binary:\t' + getChunks(hex2Bin(memLocation.replace(/\s+/g, '')), 4).join(' '))
        const rearranged = rearrangeBin(hex2Bin(memLocation.replace(/\s+/g, '')), layoutMode.layout)
