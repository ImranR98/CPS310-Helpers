...
const getCompiledPostByte = (assembly) => {
    ...
    for (let [c, a] of data.postBytes) {
        ...
        else if (a.length === assembly.length && a.indexOf('nn') >= 0) {
            const variableStartIndex = a.split('m').join('n').indexOf('n')
            const variableEndIndex = a.split('m').join('n').lastIndexOf('n') + 1
            if (
                a.slice(0, variableStartIndex) === assembly.slice(0, variableStartIndex) &&
                a.slice(variableEndIndex) === assembly.slice(-1 * (a.length - variableEndIndex))
            ) {
                const numBytesRemaining = Number.parseInt((variableEndIndex - variableStartIndex) / 2)
                let dec = assembly.slice(variableStartIndex, -1 * (a.length - variableEndIndex))
                let fin = ensureHex(dec, numBytesRemaining)
