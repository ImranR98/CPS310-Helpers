    ...
    if (remaining) {
        ...
        if (remaining.startsWith('#')) {
            ...
            if (!opCodeCompiled) { // Best guess?
                opCodeCompiled = Object.keys(data.addressingModes).concat(null).map(key => { return { mode: data.addressingModes[key], opCode: getCompiledOpCode(input[0], key ? data.addressingModes[key] : null) } }).filter(res => !!res.opCode)[0]
                let bestGuessMode = opCodeCompiled.mode
                opCodeCompiled = opCodeCompiled.opCode
                if (bestGuessMode === data.addressingModes.relative) opCodeCompiled = 'NOTE! Relative opCode but non-relative data! Subtract PC from provided data!\n' + opCodeCompiled
            }
...
module.exports = { getAssemblyOpCode, getCompiledOpCode, getAssemblyPostByte, getCompiledPostByte, getAssemblyFull, getCompiledFull, hex2Dec, ensureHex }